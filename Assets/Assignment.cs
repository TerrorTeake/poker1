...
public class Assignment : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        {
            WinState state;
            GetPlayerHands(str, out p1Hand, out p2Hand);
            Card.CheckVictory(p1Hand, p2Hand, out state);
            i++;
        }
        Debug.Log("p1Wins " + PlayerInfo.player1Wins + " p2Wins " + PlayerInfo.player2Wins + " Ties " + PlayerInfo.ties);
    }

    public void GetPlayerHands(string roundString, out Card[] p1Hand, out Card[] p2Hand)
    {
        string[] playerHands = roundString.Split('|');

        string[] player1Cards = playerHands[0].Split(' ');
        Card[] player1Hand = new Card[5];
        for (int i = 0; i < player1Hand.Length; i++)
        {
            player1Hand[i] = StringToCard(player1Cards[i]);
        }
        p1Hand = player1Hand;

        string[] player2Cards = playerHands[1].Split(' ');
        Card[] player2Hand = new Card[5];
        for (int i = 0; i < player2Hand.Length; i++)
        {
            player2Hand[i] = StringToCard(player2Cards[i]);
        }
        p2Hand = player2Hand;
    }
